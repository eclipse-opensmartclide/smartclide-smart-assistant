...
class CodeProcess:
    ...
    def get_package_path(self) -> str:
        from autocomplete import getPackagePath
...
class CodePreProcess(CodeProcess):
    ...
    def clean_comment_code_line(self, inputLine: str) -> str:
        import re
        comment_placeholder = self.lexical_placeholder['comment']

        filteredInput = re.sub("(/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*+/)|(//.*)", comment_placeholder,
                               inputLine)  # for remove /*comments
        filteredInput2 = re.sub("/\*(.*)?", comment_placeholder, filteredInput)  # for remove /*comments
        # filteredInput3=re.sub("\*[\s]?.*", comment_placeholder, filteredInput2) #for remove *comment
        filteredInput4 = re.sub("\*[\s]?@.*", comment_placeholder,
                                filteredInput2)  # for remove *@test but keep Type Annotations in Java 8:@override |@interface,.
        ...
        return filteredInput5

    def clean_comment_dataset(self, target_clmn, des_clmn) -> DataFrame:
        self.df[des_clmn] = self.df[target_clmn].astype(str).apply(self.clean_comment_code_line)
        return self.df

    # TODO most common string
    def find_string_frq(self, inputLine: str) -> str:
        import re
        pat_doublequote = r'["][\w\s\d\n$!?#$%&\'()*+,-./:;<=>?@[\]^_`{|}~]+["]'
        match = re.search(pat_doublequote, inputLine)
        if match is None:
            return '-+-'
        else:
            return match[0]

    def find_string_frq_df(self, target_clmn, des_clmn) -> DataFrame:
        self.df['string'] = self.df[self.target_clmn].astype(str).apply(self.find_string_frq)
        return self.df


    def clean_string(self, inputLine) -> str:
        string_placeholder = self.lexical_placeholder['string']
        pat_doublequote = r'["][\w\s\d\n$!?#$%&\'()*+,-./:;<=>?@[\]^_`{|}~]+["]'
        filteredInput = re.sub(pat_doublequote, string_placeholder, inputLine)  # for remove /*comments
        return filteredInput

    def clean_string_dataset(self, target_clmn, des_clmn) -> DataFrame:
        self.df[des_clmn] = self.df[target_clmn].astype(str).apply(self.clean_string)
        return self.df
